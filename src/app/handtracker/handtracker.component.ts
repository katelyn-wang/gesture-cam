...
export class HandtrackerComponent implements OnInit {
  ...
  */
  ...
  runningDetection: boolean = false;
  ...
  startDetection(){
    this.startVideo().then(()=>{
        ...
        this.video.nativeElement.style.height = "0"
        console.log("starting predictions");
        this.runInterval = setInterval(()=>{
            this.runDetection();
        }, this.SAMPLERATE);
        this.runningDetection = true;
  ...
  stopDetection(){
    ...
    this.runningDetection = false;
    this.onPrediction.emit(new PredictionEvent(""));
  }
    ...
    if (this.model != null){
        let predictions = this.model.detect(this.video.nativeElement).then((predictions: any) => {
            ...
            for(let p of predictions){
                ...
                if(p.label == 'point'){
                  if ((p.bbox[0] + p.bbox[2])/2 >= 200) rightPoint++;
                  else leftPoint++;
                }
            ...
            else if (rightPoint > 0) this.detectedGesture = "Right Point";
            else if (leftPoint > 0) this.detectedGesture = "Left Point";
